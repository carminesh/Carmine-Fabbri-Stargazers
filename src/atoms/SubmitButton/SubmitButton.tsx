...
const SubmitButton: React.FC<SubmitButtonProps> = ({ testID, icon, isButtonEnabled, onPressCallback }) => {
    const dimensions = useWindowDimensions();
    const styles = style(dimensions);

    return (
        <TouchableOpacity testID={testID ? testID : 'submit-button'} style={[styles.buttonContainer, isButtonEnabled && { opacity: 0.7 }]} disabled={isButtonEnabled} onPress={() => onPressCallback()}>
